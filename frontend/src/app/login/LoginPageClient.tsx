...
export default function LoginPageClient() {
  ...
  async function handleLogin(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // keep it simple: try to read a message, else fallback
      const message =
        typeof err === "object" &&
        err !== null &&
        "response" in err &&
      ...
      <div className="relative w-full max-w-sm flex flex-col items-center">
        ...
        <div className="w-full rounded-2xl shadow-lg border border-neutral-300 bg-white mt-64 p-6 pt-6 z-10 relative">
          <form onSubmit={handleLogin} className="flex flex-col gap-4">
            ...
            <button
              type="submit"
              className="w-full bg-[#192752] text-white rounded-lg py-2 font-semibold hover:bg-[#192752]/90 transition"
            >
              Login
            </button>
          </form>
        ...
        <p className="text-center text-sm text-[#192752] mt-4">
          ...
          <a
            href="/register"
            className="text-[#192752] font-semibold hover:underline"
          >
            Register
          </a>
        </p>
